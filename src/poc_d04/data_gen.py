...
# ---------------------------------------------------------------- 후보 B·C 조별 실습용 가상 자료 (로직 없음, 양식·데이터만)

QUOTES_B = [
    # 업체, 품목ID, 표기 품명, 단가(원), 단가 단위, 포장 단위, 배송비 조건, 납기, 비고
    ["가나상사", "P-03", "PVC 파이프 50A", "8,500", "본", "10본/묶음", "50만원 이상 무료", "발주 후 3일", ""],
    ["다라자재", "P-03", "PVC관 50A", "1,750", "m", "4m/본", "별도 청구", "발주 후 2일", "단가 단위가 m → 본 환산 필요"],
    ["마바건자재", "P-03", "PVC 파이프", "8,200", "본", "미기재", "미기재", "협의", "규격·배송비 미기재 → 비교 불가 표시"],
    ["가나상사", "P-01", "볼밸브 50A", "12,000", "개", "1개", "50만원 이상 무료", "발주 후 3일", ""],
    ["다라자재", "P-01", "볼밸브", "11,500", "EA", "1EA", "현장 직납 시 3만원", "발주 후 5일", "규격 미기재(50A/100A?) → 확인"],
    ["마바건자재", "P-01", "볼밸브 50A", "10,900", "개", "10개/박스", "박스 단위만 무료", "발주 후 4일", "포장 단위 조건 확인"],
]

ORDERS_C = [
    # 발주번호, 품목ID, 품명, 현장, 발주수량, 단위, 입고1(일자/수량), 입고2, 입고3, 갱신일, 갱신자, 비고
    ["PO-2609-001", "P-03", "PVC 파이프 50A", "A현장", "40", "본", "09-10 / 20", "09-14 / 20", "", "2026-09-14", "박지원", "완납"],
    ["PO-2609-002", "P-01", "볼밸브 50A", "A현장", "20", "개", "09-12 / 10", "", "", "2026-09-12", "박지원", "잔량 10 — 2차 입고 예정일 미기재"],
    ["PO-2609-003", "P-09", "배관 보온재 50A", "B현장", "120", "m", "09-11 / 60", "09-15 / 60", "", "2026-09-11", "정설비", "2차 입고 후 갱신 안 됨(갱신일이 1차 시점) → 미갱신"],
    ["PO-2609-004", "P-10", "테프론 테이프 12mm", "C현장", "10", "롤", "", "", "", "2026-09-05", "한구매", "발주 12일 경과, 입고 기록 없음 → 확인"],
    ["PO-2609-005", "P-05", "강관 50A", "B현장", "30", "본", "09-13 / 30", "", "", "", "", "입고는 있으나 갱신일·갱신자 공란 → 책임자 확인"],
]


def write_bc_fixtures(out_dir: Path) -> tuple[Path, Path]:
    pb = out_dir / "D04_가상견적서_후보B.csv"
    with pb.open("w", newline="", encoding="utf-8-sig") as f:
        w = csv.writer(f)
        w.writerow(["업체", "품목ID", "표기 품명", "단가(원)", "단가 단위", "포장 단위", "배송비 조건", "납기", "비고(교육용 함정 설명)"])
        w.writerows(QUOTES_B)
    pc = out_dir / "D04_발주입고_후보C.csv"
    with pc.open("w", newline="", encoding="utf-8-sig") as f:
        w = csv.writer(f)
        w.writerow(["발주번호", "품목ID", "품명", "현장", "발주수량", "단위", "입고1(일자/수량)", "입고2", "입고3", "갱신일", "갱신자", "비고(교육용 함정 설명)"])
        w.writerows(ORDERS_C)
    return pb, pc
